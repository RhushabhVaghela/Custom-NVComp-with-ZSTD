...
🔥 UPDATED: Attempting to use 'compression_level' based on new documentation
*/
...
#include <nvcomp/zstd.h>                // 🔥 ADDED: For nvcompBatchedZstdCompressDefaultOpts
...
torch::Tensor jit_compress_zstd_v1_launcher(torch::Tensor input_tensor, int zstd_level) {
  std::cout << "[JIT Kernel .cu] 🔥 Launching GPU Zstd COMPRESSION (nvCOMP)..." << std::endl;
  ...
  try {
    ...
    // --- START: KERNEL FIX (Attempting .compression_level) ---
    // This logic is based on the official documentation snippets.
    std::cout << "[JIT Kernel .cu] Attempting to set Zstd compression level: " 
              << zstd_level << std::endl;

    // 1. Start with the default options
    nvcompBatchedZstdCompressOpts_t compress_opts = nvcompBatchedZstdCompressDefaultOpts;
    
    // 2. Set the compression level
    compress_opts.compression_level = zstd_level; 
    
    // 3. Use default decompression options
    nvcompBatchedZstdDecompressOpts_t decompress_opts = nvcompBatchedZstdDecompressDefaultOpts;

    // 4. Construct the manager with the custom options
    nvcomp::ZstdManager manager(
        chunk_size,
        compress_opts,      // Pass the custom struct
        decompress_opts,
        raw_stream
    );
    // --- END: KERNEL FIX ---

    std::cout << "[JIT Kernel .cu] nvCOMP Manager created successfully" << std::endl;
    ...
    std::cout << "[JIT Kernel .cu] Max compressed size: " << format_bytes_cpp_host(max_compressed_size) << std::endl;
    ...
    std::cout << "[JIT Kernel .cu] ✅ GPU Compression complete. Compressed size: "
              << format_bytes_cpp_host(h_output_size) << std::endl;
    
    return output_buffer.slice(0, 0, h_output_size).clone();
    
  } catch (const std::exception& e) {
    std::cerr << "[JIT Kernel .cu] ❌ nvCOMP Compression error: " << e.what() << std::endl;
    throw;
  }
}
...
torch::Tensor jit_decompress_zstd_v1_launcher(
  ...
  std::cout << "[JIT Kernel .cu] 🔥 Launching GPU Zstd DECOMPRESSION (nvCOMP)..." << std::endl;
  ...
  try {
    ...
    std::cout << "[JIT Kernel .cu] nvCOMP Manager created successfully for decompression" << std::endl;
    ...
    std::cout << "[JIT Kernel .cu] ✅ GPU Decompression complete. Output size: "
              << format_bytes_cpp_host(uncompressed_size_bytes) << std::endl;
    
    return output_buffer;
    
  } catch (const std::exception& e) {
    std::cerr << "[JIT Kernel .cu] ❌ nvCOMP Decompression error: " << e.what() << std::endl;
    throw;
  }
}
