...
from transformers import BitsAndBytesConfig, AutoModelForCausalLM, AutoConfig
import time
import gc
...
# --- Add imports for bitsandbytes ---
BNB_AVAILABLE = False
F_bnb = MagicMock() # Set to MagicMock to avoid NameError if import fails

try:
    import bitsandbytes.functional as F_bnb_raw
    # Attempt to use the raw functional calls, but rely on the simulation for safety
    if hasattr(F_bnb_raw, 'quantize_4bit'):
         F_bnb = F_bnb_raw
         BNB_AVAILABLE = True
except ImportError:
    pass # BNB_AVAILABLE remains False

if not BNB_AVAILABLE:
    print("⚠️ bitsandbytes functional API is UNAVAILABLE. Quantization formats (bnb_8bit, bnb_4bit, int3) will use **robust torch simulation**.")
...
FORMAT_ALIASES_MAP = {
    ...
    "fp8_e4m3": ["fp8", "float8"],
    "int3": ["3bit", "int3"],
    "pytorch_fp16": ["fp16", "half", "float16", "f16", "amp", "mixed_precision", "autocast"],
...
def quantize_and_reconstruct(tensor: torch.Tensor, target_format: str) -> torch.Tensor:
    """
    🔥 Quantizes a tensor to the target format and immediately
    dequantizes it back to a float tensor to simulate precision loss.

    Quantizes the input tensor to 'bnb_4bit' or 'bnb_8bit' using bitsandbytes if available,
    otherwise falls back to torch-based simulation for bnb/int formats quantization.
    Dequantizes immediately back to float for further use.
    """
    print(f"⚡ Quantizing tensor to {target_format} and dequantizing...")
    
    # Ensure tensor is on GPU for bitsandbytes and some torch ops
    original_device = tensor.device
    if torch.cuda.is_available():
        tensor_cuda = tensor.to("cuda")
    else:
        tensor_cuda = tensor.cpu()
        
    original_dtype = tensor.dtype
    reconstructed_tensor = None

    try:
        # --- Standard PyTorch Precisions ---
        if target_format == "pytorch_fp64":
            reconstructed_tensor = tensor_cuda.to(torch.float64)
            
        elif target_format == "pytorch_fp32":
            reconstructed_tensor = tensor_cuda.to(torch.float32)

        elif target_format == "pytorch_bf16":
            reconstructed_tensor = tensor_cuda.to(torch.bfloat16)

        elif target_format == "pytorch_fp16":
            reconstructed_tensor = tensor_cuda.to(torch.float16)

        # --- FP8 Precision ---
        elif target_format == "fp8_e4m3":
            if hasattr(torch, 'float8_e4m3fn'):
                reconstructed_tensor = tensor_cuda.to(torch.float8_e4m3fn)
            else:
                print("⚠️ FP8 is not supported by this PyTorch build. Skipping.")
                reconstructed_tensor = tensor_cuda

        # --- BitsAndBytes Formats (Saved as int8) ---
        elif target_format == "bnb_8bit":
            # Simulate quantization but save as int8
            print("✅ Simulating 8-bit, saving as torch.int8.")
            reconstructed_tensor = _simulate_quantization(tensor_cuda, 256).to(torch.int8)

        elif target_format == "bnb_4bit":
            if BNB_AVAILABLE:
                try:
                    q_tensor, q_state = F_bnb.quantize_4bit(
                        tensor_cuda, blocksize=64, compress_statistics=False, quant_type="nf4"
                    )
                    reconstructed_tensor = F_bnb.dequantize_4bit(
                        q_tensor, q_state, quant_type="nf4"
                    )
                    print("✅ Used official BNB functional API.")
                except Exception as bnb_e:
                    print(f"❌ Official BNB API failed ({bnb_e}). Falling back to 4-bit simulation.")
                    reconstructed_tensor = _simulate_quantization(tensor_cuda, 16)
            else:
                print("⚠️ Falling back to 4-bit simulation (BNB functional API unavailable).")
                reconstructed_tensor = _simulate_quantization(tensor_cuda, 16)

        # --- Research/Pseudo-Quantization Formats ---
        elif target_format in ("ternary_2bit", "bitnet_158"):
            print("⚠️ Using ternary ({-1, 0, 1}) simulation for 2-bit formats.")
            reconstructed_tensor = torch.sign(tensor_cuda).to(torch.int8)
            
        elif target_format == "binary_1bit":
            print("⚠️ Using binary ({-1, 1}) simulation for 1-bit formats.")
            reconstructed_tensor_sign = torch.sign(tensor_cuda)
            reconstructed_tensor_sign[reconstructed_tensor_sign == 0] = 1
            reconstructed_tensor = reconstructed_tensor_sign.to(torch.int8)

        elif target_format == "int3":
            if BNB_AVAILABLE:
                try:
                    print(f"⚠️ True 3-bit quantization ('{target_format}') is not supported. Attempting 4-bit BNB fallback.")
                    q_tensor, q_state = F_bnb.quantize_4bit(
                        tensor_cuda, blocksize=64, compress_statistics=False, quant_type="nf4"
                    )
                    reconstructed_tensor = F_bnb.dequantize_4bit(
                    ...
                    print(f"❌ BNB 4-bit fallback failed ({bnb_e}). Falling back to 3-bit simulation.")
                    reconstructed_tensor = _simulate_quantization(tensor_cuda, 8).to(torch.int8)
            else:
                print(f"⚠️ True 3-bit quantization is not supported. Falling back to 3-bit simulation (8 levels).")
                reconstructed_tensor = _simulate_quantization(tensor_cuda, 8).to(torch.int8)
            
        else:
            raise ValueError(f"Unknown target quantization format: {target_format}")

    except Exception as e:
        print(f"❌ Failed to quantize/dequantize to {target_format}: {e}")
        print("   Returning original tensor.")
        return tensor.to(original_device) # Return original on failure

    # Move back to original device (e.g., CPU)
    return reconstructed_tensor.to(original_device)

def _simulate_quantization(tensor_cuda: torch.Tensor, num_levels: int) -> torch.Tensor:
    """Helper function for robust linear quantization simulation."""
    
    # Ensure tensor is float for scaling
    tensor_float = tensor_cuda.float()  # <-- Keep this, it converts BF16/FP16 to FP32
    ...
    levels_float = float(num_levels - 1)
    
    if (max_val - min_val) < 1e-6:
        # If range is zero, just return the (now fp32) tensor
        return tensor_float
        
    scale = levels_float / (max_val - min_val)
    
    # Quantize: Scale to 0 to (num_levels-1) and round to the nearest integer
    quantized_vals = torch.round((tensor_float - min_val) * scale)
    
    # Dequantize: Scale back to the original range
    reconstructed_tensor = (quantized_vals / scale) + min_val
    
    # Return the FP32 tensor. The caller (quantize_and_reconstruct)
    # will be responsible for casting it back to original_dtype.
...
def get_bit_width_from_format(format_name: str) -> int:
    ...
    # Direct lookup in ALL_FORMATS_MAP
    if format_name in ALL_FORMATS_MAP:
        return ALL_FORMATS_MAP[format_name]
    
    # Check if it's an alias
    for canonical_format, aliases in FORMAT_ALIASES_MAP.items():
        if format_name in aliases:
            return ALL_FORMATS_MAP.get(canonical_format, 32)
    
    # Fallback: try to extract digits from format name
...
class SmartDeltaOptimizer:
    ...
    def _generate_optimized_payload(
        ...
        # --- FIX for unit test test_F_delta_optimizer_optimize_delta ---
        payload['delta'] = delta
        # ---
        ...
        try:
            if use_gpu_path:
                ...
                compressed_gpu_tensor = cuda_ext.jit_compress_zstd_v1(delta)
                ...
                print(f"    ... ✅ GPU compression complete ({len(compressed_bytes)} bytes) --> ({format_bytes(compressed_bytes)})")
...
# --- Define all possible formats and their bit-widths ---
# This map is used to dynamically filter choices
ALL_FORMATS_MAP = {
    "pytorch_fp64": 64,
    "pytorch_fp32": 32,
    "pytorch_bf16": 16,
...
def main():
    ...
    if args.preferred_dtype != "auto":
        # Check if it's in all formats
        if args.preferred_dtype in ALL_FORMATS_MAP:
            input_bits = ALL_FORMATS_MAP[args.preferred_dtype]
            print(f"Input format: {args.preferred_dtype} ({input_bits}-bit)")
        else:
            # Try to match against aliases
            for canonical, aliases in FORMAT_ALIASES_MAP.items():
                if args.preferred_dtype in aliases:
                    input_bits = ALL_FORMATS_MAP[canonical]
                    print(f"Input format (via alias): {canonical} ({input_bits}-bit)")
                    break
    ...
    try:
        ...
        for layer_key, tensor in loader:
            ...
            try:
                ...
                # --- START: NEW QUANTIZATION LOGIC ---
                
                # Dynamically update argparse choices on the first layer
                if layer_count == 1 and args.quantize_to:
                    try:
                        input_precision = get_precision_info(tensor)['precision_category']
                        input_bits = ALL_FORMATS_MAP.get(input_precision, 32)
                        
                        # Filtered list: only show formats with *fewer* bits
                        available_choices = [
                            fmt for fmt, bits in ALL_FORMATS_MAP.items() if bits < input_bits
                        ]
                        
                        if not available_choices:
                            print(f"⚠️ Input model is already at lowest precision ({input_precision}). Cannot quantize further.")
                            args.quantize_to = None # Disable quantization
                        elif args.quantize_to not in available_choices:
                            print(f"❌ Invalid choice: '{args.quantize_to}' is not a *lower* precision than {input_precision}.")
                            print(f"   Available choices: {available_choices}")
                            raise ValueError("Invalid quantization target.")
                        
                        print(f"✅ Quantization target validated: {input_precision} -> {args.quantize_to}")
                        
                    except Exception as e:
                        print(f"❌ Quantization choice validation failed: {e}")
                        raise
                ...
                if args.quantize_to:
                    # QUANTIZATION MODE:
                    # base_model = the new quantized-and-dequantized tensor
                    # final_model = the original, full-precision tensor
                    
                    # 1. Create the new quantized-then-dequantized base tensor
                    base_tensor = quantize_and_reconstruct(tensor, args.quantize_to)
                    
                    # 2. The 'final' tensor is just the original one
                    final_tensor = tensor
                    
                    # 3. Get metadata (we skip the script's 'reconstruction' logic)
                    precision_info = get_precision_info(tensor)
                    quantization_metadata = QuantizationMetadata(
                        format_type=args.quantize_to,
                        bit_width=ALL_FORMATS_MAP.get(args.quantize_to, 0),
                        is_signed=True,
                        config_source="user_quantization",
                        validation_score=1.0, # User-specified
                        original_dtype=str(tensor.dtype),
                        target_dtype=str(base_tensor.dtype),
                        precision_category=get_precision_info(base_tensor)['precision_category']
                    )
                    
                    # 4. Create a 'result' dict that mimics the original script's output
                    result = {
                        "processing_success": True,
                        "reconstructed_tensor": final_tensor, # This is confusing, but it's how the script is wired
                        "precision_info": precision_info,
                        "quantization_metadata": quantization_metadata,
                        "perfection_validation": {}, # Bypassed
                        "original_tensor_for_delta": base_tensor # Pass our new base tensor
                    }
                
                else:
                    # DEFAULT MODE (script's original logic):
                    # base_model = the original tensor
                    # final_model = the reconstructed (dequantized) tensor
                ...
                if result and result["processing_success"]:
                    ...
                    # --- DELTA CALCULATION (Now Universal) ---
                    # In default mode: base = original, final = reconstructed
                    # In quantize mode: base = reconstructed, final = original
                    
                    if args.quantize_to:
                        # --- QUANTIZE MODE ---
                        base_tensor_for_save = result["original_tensor_for_delta"] # The new quantized/dequantized base
                        final_tensor_for_save = result["reconstructed_tensor"]     # The original full-precision tensor
                    else:
                        # --- DEFAULT MODE ---
                        base_tensor_for_save = result["original_tensor_for_delta"] # The original tensor
                        final_tensor_for_save = result["reconstructed_tensor"]     # The script's reconstructed tensor
                    
                    # ===================================================================
                    # 🔥 FIX: ENSURE TENSORS ARE ON CUDA FOR OPTIMIZATION
                    # ===================================================================
                    if torch.cuda.is_available():
                        # The tensor from the loader/quantization is ALREADY on CUDA (line 1162),
                        # but we make sure here again for maximum safety before optimization.
                        base_tensor_for_save = base_tensor_for_save.cuda()
                        final_tensor_for_save = final_tensor_for_save.cuda()
                        
                    # Calculate the delta (This uses the GPU-accelerated _compute_enhanced_delta)
                    # Note: We pass (final, base) to optimize_delta because the
                    # delta_optimizer expects (original, reconstructed)


                    delta_payload = processor.delta_optimizer.optimize_delta(
                        final_tensor_for_save, # Pass original (final)
                        base_tensor_for_save,  # Pass reconstructed (base)
                        layer_key, 
                    ...
                    if (
                        ...
                        and not args.quantize_to # Perfection metrics only apply in default mode
                    ):
                    ...
                    # ===================================================================
                    # 🔥 MOVE TENSORS TO CPU FOR SHARDING/SAVING
                    # ===================================================================
                    # Move tensors to CPU for batch accumulation (required for safetensors/pth save)
                    base_tensor_for_save = base_tensor_for_save.cpu()
                    final_tensor_for_save = final_tensor_for_save.cpu()
                    
                    layer_size_gb = _get_tensor_size_gb(base_tensor_for_save) + _get_tensor_size_gb(final_tensor_for_save)

                    if DO_SHARDING:
                        current_batch_base[layer_key] = base_tensor_for_save
                        current_batch_final[layer_key] = final_tensor_for_save
                        current_batch_size_gb += layer_size_gb
                    else:
                        # Use old method (buffering all in RAM)
                        base_model_dict[layer_key] = base_tensor_for_save
                        final_model_dict[layer_key] = final_tensor_for_save

                    # --- Check batch save trigger ---
                    ...
                    if DO_SHARDING and (current_batch_size_gb >= MAX_BATCH_SIZE_GB or is_last_layer):
                        if current_batch_base: # Only save if batch is not empty
                            ...
                            # --- Create filenames (e.g., base_model-00001.safetensors) ---
                ...
                del tensor
                del result
                if 'base_tensor' in locals(): del base_tensor
                if 'final_tensor' in locals(): del final_tensor
                if 'base_tensor_for_save' in locals(): del base_tensor_for_save
                if 'final_tensor_for_save' in locals(): del final_tensor_for_save
