...
void run_test(const std::string &name, const std::vector<uint8_t> &input,
  ...
  CHECK_STATUS(manager.compress_async(
      ...
      d_compressed_ptrs_array, d_compressed_size, d_temp, temp_size));
  CHECK_CUDA(cudaDeviceSynchronize());

  size_t compressed_size;
  CHECK_CUDA(cudaMemcpy(&compressed_size, d_compressed_size, sizeof(size_t),
  ...
  CHECK_STATUS(manager.decompress_async(
      ...
      d_decomp_ptrs_array, d_decomp_size, d_temp, decomp_temp_size));
  CHECK_CUDA(cudaDeviceSynchronize());

  // Verify
  std::vector<uint8_t> output(input_size);
  CHECK_CUDA(
      cudaMemcpy(output.data(), d_decomp, input_size, cudaMemcpyDeviceToHost));
...
int main() {
  ...
  // run_test("Empty Input", {}, false); // Skip for now, might need special
  // handling
