...
void test_rle_roundtrip() {
    ...
    // Check output size
    u32 output_size = 0;
    CUDA_CHECK(cudaMemcpy(&output_size, d_output_sizes, sizeof(u32), cudaMemcpyDeviceToHost));
    printf("Output size: %u bytes\n", output_size);
    
    // RLE header is 14 bytes.
    if (output_size != 14) {
        printf("❌ Expected output size 14 (Header+Symbol), got %u\n", output_size);
        // It might be acceptable if logic differs slightly, but for RLE it should be small.
        if (output_size > 100) {
    ...
    CUDA_CHECK(cudaMemcpy(h_decoded.data(), d_decoded, data_size, cudaMemcpyDeviceToHost));
...
void test_zero_byte_input() {
    ...
    if (status == Status::SUCCESS) {
        ...
        printf("⚠️ 0-Byte Input returned status: %d (Might be expected if not supported)\n", (int)status);
    }
