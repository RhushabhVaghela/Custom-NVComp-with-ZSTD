...
bool verify_decompressed_data(const uint8_t *original,
...
bool test_single_chunk_streaming() {
  ...
  if (!safe_cuda_memcpy(d_input, h_input.data(), chunk_size,
  ...
  status = manager.compress_chunk(d_input, chunk_size, d_compressed,
  ...
  // Decompress
  size_t decompressed_size = chunk_size;
  bool is_last;
  ...
  if (!safe_cuda_memcpy(h_output.data(), d_output, chunk_size,
  ...
  ASSERT_TRUE(
      verify_decompressed_data(h_input.data(), h_output.data(), chunk_size),
...
bool test_multi_chunk_streaming() {
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    LOG_INFO("Chunk " << i << ": " << chunk_size << " -> " << compressed_size
                      ...
                      << get_compression_ratio(chunk_size, compressed_size)
                      << ":1)");
  }

  LOG_INFO("Total compressed: " << total_compressed << " bytes");
  LOG_INFO("Overall ratio: "
           << std::fixed << std::setprecision(2)
  ...
  // Verify
  ASSERT_TRUE(
      verify_decompressed_data(h_input.data(), h_output.data(), total_size),
...
bool test_variable_chunk_sizes() {
  ...
  std::vector<size_t> chunk_sizes = {1024, 4096, 16384, 65536,
                                     1024}; // 1KB, 4KB, 16KB, 64KB, 1KB
  size_t total_size = 0;
  ...
  for (size_t i = 0; i < chunk_sizes.size(); i++) {
    ...
    size_t decompressed_size = chunk_sizes[i];
    bool is_last;
    Status status =
        manager.decompress_chunk(d_compressed, compressed_chunks[i].size(),
                                 d_output, &decompressed_size, &is_last);
    ASSERT_STATUS(status, "Variable chunk " << i << " decompression failed");
    ASSERT_EQ(decompressed_size, chunk_sizes[i],
              "Size mismatch for chunk " << i);

    cudaMemcpy(h_output.data() + offset, d_output, chunk_sizes[i],
               cudaMemcpyDeviceToHost);
...
bool test_cross_chunk_matching() {
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    LOG_INFO("Chunk " << i << ": " << compressed_size
                      << " bytes (ratio: " << std::fixed << std::setprecision(2)
                      << get_compression_ratio(chunk_size, compressed_size)
                      << ":1)");
  }
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    cudaMemcpy(h_output.data() + (i * chunk_size), d_output, decompressed_size,
               cudaMemcpyDeviceToHost);
  }
...
bool test_large_streaming() {
  ...
  for (int i = 0; i < num_chunks; i++) {
    // Generate chunk data
    generate_test_data(h_input, chunk_size,
                       (i % 3 == 0) ? "text" : "repetitive");
...
bool test_incompressible_data_streaming() {
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    LOG_INFO("Chunk " << i << ": ratio " << std::fixed << std::setprecision(3)
                      << ratio << ":1"
  ...
  LOG_INFO("Overall ratio: "
           ...
           << get_compression_ratio(chunk_size * num_chunks, total_compressed)
           << ":1");
...
bool test_mixed_compressibility() {
  ...
  const char *patterns[] = {"repetitive", "random", "text", "sequential",
                            "repetitive"};
  const int num_chunks = 5;
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    LOG_INFO("Chunk " << i << " (" << patterns[i] << "): " << chunk_size
                      << " -> " << compressed_size
                      << " bytes (ratio: " << std::fixed << std::setprecision(2)
                      << get_compression_ratio(chunk_size, compressed_size)
                      << ":1)");
...
int main() {
  ...
  total++;
  if (test_single_chunk_streaming())
    passed++;
  total++;
  if (test_multi_chunk_streaming())
    passed++;
  total++;
  if (test_variable_chunk_sizes())
    passed++;
  ...
  total++;
  if (test_very_small_chunks())
    passed++;
  total++;
  if (test_large_streaming())
    passed++;
  total++;
  if (test_incompressible_data_streaming())
    passed++;
  total++;
  if (test_mixed_compressibility())
    passed++;
