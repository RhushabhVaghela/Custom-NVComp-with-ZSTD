...
void clear_cuda_errors() {
  ...
  if (err != cudaSuccess) {
    ...
    if (err == cudaErrorIllegalAddress) {
      std::cerr
          << "[RECOVERY] Performing cudaDeviceReset() to clear corruption..."
          << std::endl;
      cudaDeviceReset();
      std::cerr << "[RECOVERY] Device reset complete" << std::endl;
    }
  }
  // Clear the error - allows test framework to continue even if GPU is in bad
  // state
  cudaGetLastError();

  // NOTE: Removed cudaDeviceReset() because it invalidates the global stream
  // pool singleton which causes subsequent tests to fail with "invalid device
  // context".
}
...
bool verify_data_exact(const uint8_t *original, const uint8_t *decompressed,
                       size_t size) {
...
void generate_pattern_data(std::vector<uint8_t> &data, size_t size,
                           uint8_t pattern) {
  data.resize(size);
  for (size_t i = 0; i < size; i++) {
    data[i] = pattern;
...
bool test_identity_property() {
  ...
  size_t compressed_size =
      data_size * 2; // Set to buffer capacity (required by compress API)
  Status status =
  ...
  size_t decompressed_size =
      data_size; // Set to buffer capacity (required by decompress API)
...
bool test_random_inputs_roundtrip() {
  ...
  for (int test = 0; test < num_tests; test++) {
    ...
    // Copy input data to device
    if (!safe_cuda_memcpy(d_input, h_input.data(), size,
                          cudaMemcpyHostToDevice)) {
    ...
    if (status == Status::SUCCESS) {
      size_t decompressed_size =
          size; // BUGFIX: Initialize to output buffer capacity
      status = manager->decompress(d_compressed, compressed_size, d_output,
                                   &decompressed_size, d_temp, temp_size);

      if (status == Status::SUCCESS && decompressed_size == size) {
        std::vector<uint8_t> h_output(size);
        if (!safe_cuda_memcpy(h_output.data(), d_output, size,
                              cudaMemcpyDeviceToHost)) {
...
bool test_all_compression_levels() {
  ...
  for (int level = 1; level <= 22; level++) {
    ...
    size_t compressed_size =
        data_size * 2; // BUGFIX: Initialize to buffer capacity
    Status status =
        manager->compress(d_input, data_size, d_compressed, &compressed_size,
                          d_temp, temp_size, nullptr, 0, 0);
    ...
    if (status == Status::SUCCESS) {
      size_t decompressed_size =
          data_size; // BUGFIX: Initialize to output buffer capacity
      status = manager->decompress(d_compressed, compressed_size, d_output,
                                   &decompressed_size, d_temp, temp_size);
      ...
      if (status == Status::SUCCESS) {
        ...
        bool verified =
            verify_data_exact(h_input.data(), h_output.data(), data_size);
        float ratio = get_compression_ratio(data_size, compressed_size);

        std::cout << "  " << std::setw(5) << level << " | " << std::setw(7)
...
bool test_various_sizes_roundtrip() {
  ...
  std::vector<size_t> test_sizes = {
      1,      2,      3,      7,      15,      16,      17,     31,     32,
      33,     63,     64,     65,     127,     128,     129,    255,    256,
      257,    511,    512,    513,    1023,    1024,    1025,   4095,   4096,
      4097,   8191,   8192,   8193,   16383,   16384,   16385,  32767,  32768,
      32769,  65535,  65536,  65537,  131071,  131072,  131073, 262143, 262144,
      262145, 524287, 524288, 524289, 1048575, 1048576, 1048577};
  ...
  if (target_size_str != nullptr) {
    ...
    std::cout << "  [INFO] Debugging specific size: " << target_size
              << std::endl;
  } else if (!run_heavy) {
  ...
  for (size_t size : test_sizes) {
    ...
    size_t compressed_size = std::max(
        size * 2,
        (size_t)1024); // BUGFIX: Initialize to buffer capacity (min 1024)
    Status status =
        manager->compress(d_input, size, d_compressed, &compressed_size, d_temp,
                          temp_size, nullptr, 0, 0);
    ...
    if (status == Status::SUCCESS) {
      ...
      if (status == Status::SUCCESS && decompressed_size == size) {
        ...
        if (verify_data_exact(h_input.data(), h_output.data(), size)) {
          ...
          std::cout << "  [FAIL] Size " << size << " (Data Mismatch)"
                    << std::endl;
        }
      } else {
        std::cout << "  [FAIL] Size " << size
                  << " (Decompress Status: " << (int)status
                  << ", Size: " << decompressed_size << ")" << std::endl;
      }
    } else {
      std::cout << "  [FAIL] Size " << size
                << " (Compress Status: " << (int)status << ")" << std::endl;
    }
...
bool test_frame_format_validation() {
  ...
  if (!safe_cuda_memcpy(h_compressed.data(), d_compressed, compressed_size,
                        cudaMemcpyDeviceToHost)) {
    LOG_FAIL("test_frame_format_validation",
             "CUDA memcpy from d_compressed failed");
    safe_cuda_free(d_input);
    safe_cuda_free(d_compressed);
    safe_cuda_free(d_temp);
    return false;
...
bool test_checksum_validation() {
  ...
  if (!safe_cuda_memcpy(h_compressed.data(), d_compressed, compressed_size,
                        cudaMemcpyDeviceToHost)) {
    LOG_FAIL("test_checksum_validation",
             "CUDA memcpy from d_compressed failed");
    safe_cuda_free(d_input);
    safe_cuda_free(d_compressed);
    safe_cuda_free(d_output);
...
bool test_special_patterns() {
  ...
  std::vector<PatternTestCase> cases = {
      {"All zeros",
       [](auto &d, size_t s) { generate_pattern_data(d, s, 0x00); }},
      {"All ones",
       [](auto &d, size_t s) { generate_pattern_data(d, s, 0xFF); }},
      {"All 0xAA",
       [](auto &d, size_t s) { generate_pattern_data(d, s, 0xAA); }},
      {"All 0x55",
       [](auto &d, size_t s) { generate_pattern_data(d, s, 0x55); }},
  ...
  for (const auto &test_case : cases) {
    ...
    float ratio = get_compression_ratio(data_size, compressed_size);
    LOG_INFO("  " << test_case.name << ": " << std::fixed
                  << std::setprecision(2) << ratio << ":1 ratio");
...
bool test_byte_alignment() {
  ...
  LOG_INFO("Passed: " << passed << "/" << unaligned_sizes.size()
                      << " unaligned sizes");
...
bool test_deterministic_compression() {
  ...
  void *d_input = nullptr, *d_compressed1 = nullptr, *d_compressed2 = nullptr,
       *d_temp = nullptr;
  ...
  manager->compress(d_input, data_size, d_compressed1, &compressed_size1,
                    d_temp, temp_size, nullptr, 0, 0);
  manager->compress(d_input, data_size, d_compressed2, &compressed_size2,
                    d_temp, temp_size, nullptr, 0, 0);
  ...
  if (!safe_cuda_memcpy(h_comp1.data(), d_compressed1, compressed_size1,
                        cudaMemcpyDeviceToHost)) {
    LOG_FAIL("test_deterministic_compression",
             "CUDA memcpy from d_compressed1 failed");
    safe_cuda_free(d_input);
    safe_cuda_free(d_compressed1);
    safe_cuda_free(d_compressed2);
    safe_cuda_free(d_temp);
  ...
  if (!safe_cuda_memcpy(h_comp2.data(), d_compressed2, compressed_size2,
                        cudaMemcpyDeviceToHost)) {
    LOG_FAIL("test_deterministic_compression",
             "CUDA memcpy from d_compressed2 failed");
    safe_cuda_free(d_input);
    safe_cuda_free(d_compressed1);
    safe_cuda_free(d_compressed2);
    safe_cuda_free(d_temp);
  ...
  bool identical =
      verify_data_exact(h_comp1.data(), h_comp2.data(), compressed_size1);
  if (identical) {
...
int main(int argc, char **argv) {
  ...
  std::cout << "Failed: " << (total_count - passed_count) << "/" << total_count
            << std::endl;
