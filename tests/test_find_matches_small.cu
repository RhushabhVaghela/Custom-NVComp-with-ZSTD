...
int main() {
  ...
  if (cudaMemcpy(d_input, h_input.data(), data_size, cudaMemcpyHostToDevice) !=
      cudaSuccess) {
  ...
  // Use a debugging wrapper (if available) so we get a one-line log of the
  // top-level manager.compress call and the last CUDA error code.
  Status s =
      manager.compress(d_input, data_size, d_compressed, &compressed_size,
                       d_temp, comp_temp_size, nullptr, 0);
  auto t1 = std::chrono::high_resolution_clock::now();
  long long elapsed_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
  std::cerr << "compress took " << elapsed_ms << " ms\n";
  cudaError_t err = cudaGetLastError();
  std::cerr << "manager.compress returned status=" << status_to_string(s)
            << " cudaLast=" << err << "\n";
  ...
  // FIX: Only check Status result. cudaGetLastError() may have residual
  // errors from debug printfs (e.g. [Pipeline] messages) that don't
  // indicate actual compression failure.
