...
Status test_metadata_roundtrip() {
  ...
  try {
    ...
    if (!safe_cuda_memcpy(d_input, h_original_data.data(), DATA_SIZE,
    ...
    Status status =
        manager->compress(d_input, DATA_SIZE, d_compressed, &compressed_size,
                          d_compress_workspace, compress_workspace_size,
    ...
    std::cout << "    Frame Header Descriptor: 0x" << std::hex << (int)fhd
              << std::dec << "\n";
    ...
    if (fcs_field_size == 0 && single_segment) {
      ...
      std::cout << "    Content Size Field: " << parsed_content_size
                << " bytes (1 byte)\n";
    ...
    std::cout << " Allocated decompression workspace: "
              << decompress_workspace_size << " bytes\n\n";
    ...
    status = manager->decompress(d_compressed, compressed_size, d_decompressed,
                                 &decompressed_size, d_decompress_workspace,
