...
bool verify_data(const uint8_t* original, const uint8_t* decompressed, size_t size) {
    for (size_t i = 0; i < size; i++) {
...
bool test_dictionary_integration() {
    ...
    LOG_INFO("✓ Compressed with dictionary: " << data_size << " -> " << compressed_size);
...
bool test_batch_processing_with_memory_pool() {
    ...
    LOG_INFO("Processing " << num_items << " items of " << item_size / 1024 << " KB each");
    
    // Initialize memory pool
    memory::MemoryPoolManager pool(false);
    
    auto manager = create_batch_manager(5);
    std::vector<BatchItem> items;
    
    // Prepare batch items
    for (int i = 0; i < num_items; i++) {
        std::vector<uint8_t> h_data;
        generate_test_data(h_data, item_size, (i % 2 == 0) ? "compressible" : "text");
...
bool test_large_file_compression() {
    ...
    auto streaming_mgr = create_streaming_manager(5);
    ...
    for (int i = 0; i < num_chunks; i++) {
        ...
        Status status = streaming_mgr->compress_chunk(d_input, current_chunk, d_output,
    ...
    LOG_INFO("Time: " << std::fixed << std::setprecision(2) << elapsed_sec << " seconds");
    LOG_INFO("Throughput: " << std::fixed << std::setprecision(1)
             << (file_size / (1024.0 * 1024.0)) / elapsed_sec << " MB/s");
...
bool test_memory_efficiency_large_file() {
    ...
    for (int i = 0; i < num_chunks; i++) {
        ...
        size_t compressed_size;
        streaming_mgr->compress_chunk(d_input, current_chunk, d_output, &compressed_size,
                                      i == num_chunks - 1);
    }
...
bool test_concurrent_compression() {
    ...
    LOG_INFO("Time: " << std::fixed << std::setprecision(2) << elapsed_ms << " ms");
    LOG_INFO("Throughput: " << std::fixed << std::setprecision(1) 
...
bool test_corrupt_data_handling() {
    ...
    for (const auto& [name, data] : test_cases) {
        ...
        size_t output_size;
        Status status = manager->decompress(d_compressed, data.size(), d_output, &output_size,
                                           d_temp, data_size);
...
int main() {
    ...
    total++; if (test_complete_compression_pipeline()) passed++;
    total++; if (test_dictionary_integration()) passed++;
    total++; if (test_batch_processing_with_memory_pool()) passed++;
    ...
    total++; if (test_large_file_compression()) passed++;
    total++; if (test_memory_efficiency_large_file()) passed++;
    ...
    total++; if (test_concurrent_compression()) passed++;
    total++; if (test_race_condition_detection()) passed++;
    ...
    total++; if (test_corrupt_data_handling()) passed++;
    total++; if (test_buffer_boundary_conditions()) passed++;
