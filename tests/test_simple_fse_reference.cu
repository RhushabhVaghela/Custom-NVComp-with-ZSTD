...
struct SimpleBitstream {
    ...
    SimpleBitstream() : buffer(1024), bit_pos(0), bit_buffer(0), bits_in_buffer(0) {}
...
int main() {
    ...
    printf("Input: ");
    for (u32 i = 0; i < data_size; i++) printf("%u ", test_data[i]);
    printf("\n");
    ...
    for (u32 i = 0; i <= stats.max_symbol; i++) {
        if (stats.frequencies[i] > 0) printf("%u:%u ", i, stats.frequencies[i]);
    }
    printf("\n");
    ...
    normalize_frequencies_accurate(stats.frequencies, stats.total_count, table_size, 
                                   h_normalized.data(), stats.max_symbol, nullptr);
    
    printf("Normalized freqs: ");
    for (u32 i = 0; i <= stats.max_symbol; i++) {
        if (h_normalized[i] > 0) printf("%u:%u ", i, h_normalized[i]);
    }
    printf("\n");
    ...
    for (u32 i = 0; i < data_size; i++) {
        ...
        printf("[%u] sym=%u: state=%u -> val=%u (%u bits) -> nextIdx=%u -> state=%u\n",
    ...
    Status status = encode_fse_advanced_debug(d_input, data_size, d_output, &encoded_size, true, 0);
    ...
    CUDA_CHECK(cudaMemcpy(gpu_output.data(), d_output, encoded_size, cudaMemcpyDeviceToHost));
