...
int main() {
  ...
  if (err != cudaSuccess) {
    std::cerr << "cudaMalloc compressed failed: " << cudaGetErrorString(err)
              << std::endl;
    cudaFree(workspace);
    return 1;
  }
  ...
  // Fix: initialized compressed_size to capacity, as manager expects input
  // capacity here
  size_t compressed_size = max_compressed_size;
  auto status = manager->compress(input.data(), input_size, compressed,
                                  &compressed_size, workspace, workspace_size,
                                  nullptr, // no dict
                                  0,
                                  0 // default stream
  );
