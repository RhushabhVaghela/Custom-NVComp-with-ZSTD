...
int main() {
    ...
    const char* old = getenv("CUDA_ZSTD_STREAM_POOL_SIZE");
    setenv("CUDA_ZSTD_STREAM_POOL_SIZE", "1", 1);

    StreamPool* pool = get_global_stream_pool();
    if (!pool) {
        std::cerr << "Failed to create global pool" << std::endl;
        return 1;
    }
    ...
    // release it at end of this scope.
    {
        ...
        std::cout << "Timed acquire properly returned empty optional -> timeout" << std::endl;
    }
