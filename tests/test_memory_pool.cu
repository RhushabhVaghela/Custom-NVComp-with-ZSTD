...
bool test_pool_growth() {
    ...
    LOG_INFO("Allocating " << num_allocations << " blocks of " << test_size / 1024 << " KB");
    ...
    LOG_INFO("Total pool capacity: " << final_stats.total_pool_capacity / (1024 * 1024) << " MB");
    LOG_INFO("Current memory usage: " << final_stats.current_memory_usage / (1024 * 1024) << " MB");
    LOG_INFO("Peak memory usage: " << final_stats.peak_memory_usage / (1024 * 1024) << " MB");
...
bool test_concurrent_allocations() {
    ...
    ASSERT_EQ(success_count, num_threads * allocations_per_thread, "Some allocations failed");
...
bool test_stream_based_allocation() {
    ...
    for (int i = 0; i < num_streams; i++) {
        ...
        ASSERT_NE(ptrs[i], nullptr, "Async allocation on stream " << i << " failed");
...
bool test_allocation_overhead() {
    ...
    double pool_time = std::chrono::duration<double, std::milli>(pool_end - pool_start).count();
    ...
    double direct_time = std::chrono::duration<double, std::milli>(direct_end - direct_start).count();
    
    LOG_INFO("Pool time: " << std::fixed << std::setprecision(2) << pool_time << " ms");
    LOG_INFO("Direct cudaMalloc time: " << std::fixed << std::setprecision(2) << direct_time << " ms");
    LOG_INFO("Speedup: " << std::fixed << std::setprecision(2) << (direct_time / pool_time) << "x");
    ...
    ASSERT_TRUE(pool_time < direct_time, "Pool should be faster than direct allocation");
...
bool test_peak_memory_tracking() {
    ...
    PoolStats stats = pool.get_statistics();
    LOG_INFO("Final peak memory: " << stats.peak_memory_usage / (1024 * 1024) << " MB");
    ...
    LOG_INFO("After partial deallocation - current: " 
             << stats2.current_memory_usage / 1024 << " KB, peak: "
             << stats2.peak_memory_usage / 1024 << " KB");
...
bool test_many_allocations() {
    ...
    for (int i = 0; i < num_allocations; i++) {
        size_t size = MemoryPoolManager::SIZE_4KB * (1 << (i % 4)); // 4KB, 16KB, 64KB, 256KB
        void* ptr = pool.allocate(size);
        if (ptr != nullptr) {
            ptrs.push_back(ptr);
        }
    ...
    LOG_INFO("Allocated " << ptrs.size() << " blocks in " 
    ...
    for (int i = 0; i < num_operations; i++) {
        ...
        if ((i + 1) % 1000 == 0) {
            ...
            LOG_INFO("After " << (i + 1) << " ops: " << active_ptrs.size() 
                     << " active, hit rate: " << std::fixed << std::setprecision(1)
                     << (stats.get_hit_rate() * 100) << "%");
        }
    }
...
bool test_memory_leak_detection() {
    ...
    LOG_INFO("Initial current memory: " << initial.current_memory_usage / 1024 << " KB");
    LOG_INFO("Final current memory: " << final.current_memory_usage / 1024 << " KB");
...
int main() {
    ...
    total++; if (test_allocation_overhead()) passed++;
    total++; if (test_cache_hit_rate()) passed++;
    total++; if (test_peak_memory_tracking()) passed++;
    ...
    total++; if (test_many_allocations()) passed++;
    total++; if (test_random_allocation_pattern()) passed++;
    total++; if (test_memory_leak_detection()) passed++;
    total++; if (test_out_of_memory_handling()) passed++;
