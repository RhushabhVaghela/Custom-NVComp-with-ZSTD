...
bool test_null_pointer_detection() {
    ...
    // Null input pointer
    Status status = manager.compress(nullptr, 1024, (void*)0x1000, &dummy_size, (void*)0x2000, 1024, nullptr, 0);
    EXPECT_ERROR(status, Status::ERROR_INVALID_PARAMETER, "Null input should fail");
    LOG_INFO("✓ Null input detected");
    
    // Null output pointer
    status = manager.compress((void*)0x1000, 1024, nullptr, &dummy_size, (void*)0x2000, 1024, nullptr, 0);
    EXPECT_ERROR(status, Status::ERROR_INVALID_PARAMETER, "Null output should fail");
    LOG_INFO("✓ Null output detected");
    
    // Null size pointer
    status = manager.compress((void*)0x1000, 1024, (void*)0x3000, nullptr, (void*)0x2000, 1024, nullptr, 0);
    ...
    status = manager.compress((void*)0x1000, 1024, (void*)0x3000, &dummy_size, nullptr, 0, nullptr, 0);
...
bool test_invalid_size_parameters() {
    ...
    // Zero input size
    Status status = manager.compress(d_input, 0, d_output, &compressed_size, d_temp, 1024, nullptr, 0);
    ...
    status = manager.compress(d_input, huge_size, d_output, &compressed_size, d_temp, 1024, nullptr, 0);
    EXPECT_ERROR(status, Status::ERROR_INVALID_PARAMETER, "Huge size should fail");
    LOG_INFO("✓ Overflow size detected");
    
    // Insufficient workspace
    status = manager.compress(d_input, 1024, d_output, &compressed_size, d_temp, 1, nullptr, 0);
...
bool test_invalid_workspace_size() {
    ...
    if (!safe_cuda_malloc(&d_temp, small_size)) {
        LOG_FAIL("test_invalid_workspace_size", "CUDA malloc for small d_temp failed");
        safe_cuda_free(d_input);
        safe_cuda_free(d_output);
        return false;
    }
...
bool test_corrupted_frame_headers() {
    ...
    if (!safe_cuda_malloc(&d_compressed, data_size)) {
        LOG_FAIL("test_corrupted_frame_headers", "CUDA malloc for d_compressed failed");
        return false;
    }
    ...
    if (!safe_cuda_memcpy(d_compressed, corrupted_data.data(), data_size, cudaMemcpyHostToDevice)) {
        LOG_FAIL("test_corrupted_frame_headers", "CUDA memcpy to d_compressed failed");
        safe_cuda_free(d_compressed);
        safe_cuda_free(d_output);
        safe_cuda_free(d_temp);
        return false;
...
bool test_cleanup_on_error_paths() {
    ...
    if (!get_memory_info(free_before, total)) {
        LOG_FAIL("test_cleanup_on_error_paths", "Failed to get initial memory info");
        return false;
    }
    ...
    // Force cleanup
    // manager = ZstdBatchManager(); // Assignment to manager is invalid (deleted operator=)
    manager.set_compression_level(3);
...
bool test_memory_leak_on_failures() {
    ...
    if (!get_memory_info(free_initial, total)) {
        LOG_FAIL("test_memory_leak_on_failures", "Failed to get initial memory info");
        return false;
    }
...
bool test_cuda_error_detection() {
    ...
    // This test is inherently flaky because using invalid pointers is undefined behavior.
    // A better approach would be to trigger a known CUDA error, e.g., by
    // trying to copy to an invalid address.
    void* d_buf = nullptr;
    ...
    size_t dummy_size;
    Status status = manager.compress(d_buf, 1024, d_buf, &dummy_size, d_buf, 1024, nullptr, 0);
...
bool test_buffer_overflow_prevention() {
    ...
    // Allocate GPU memory with error checking
    if (!safe_cuda_malloc(&d_input, input_size)) {
        LOG_FAIL("test_buffer_overflow_prevention", "CUDA malloc for d_input failed");
        return false;
    }
    
    if (!safe_cuda_malloc(&d_output, 512)) {
        LOG_FAIL("test_buffer_overflow_prevention", "CUDA malloc for d_output failed");
        safe_cuda_free(d_input);
        return false;
    }
    ...
    if (!safe_cuda_malloc(&d_temp, temp_size)) {
        LOG_FAIL("test_buffer_overflow_prevention", "CUDA malloc for d_temp failed");
        safe_cuda_free(d_input);
        safe_cuda_free(d_output);
        return false;
    }
    ...
    if (!safe_cuda_memcpy(d_input, h_input.data(), input_size, cudaMemcpyHostToDevice)) {
        LOG_FAIL("test_buffer_overflow_prevention", "CUDA memcpy to d_input failed");
        safe_cuda_free(d_input);
        safe_cuda_free(d_output);
        safe_cuda_free(d_temp);
        return false;
    ...
    size_t compressed_size;
    Status status = manager.compress(d_input, input_size, d_output, &compressed_size,
                                     d_temp, manager.get_compress_temp_size(input_size), nullptr, 0);
