// Comprehensive CUDA-ZSTD Benchmark: Formulas × Block Sizes × Modes × Input Sizes
// Tests: 6 formulas × 7 block_sizes × 2 modes (serial/parallel) × 14 input_sizes
// Total: ~1,176 test combinations from 1KB to 20GB
...
std::string format_size(size_t bytes) {
    if (bytes >= 1024UL * 1024 * 1024) return std::to_string(bytes / (1024UL * 1024 * 1024)) + " GB";
...
// Benchmark runner
// =============================
bool run_benchmark(const std::string& formula_name, size_t input_size, u32 block_size, 
                   bool use_parallel, BenchmarkResult& result, int test_number) {
    result = {formula_name, input_size, block_size, use_parallel, 0.0, 0.0, false};
    
    if (input_size < 256) return false;
    
    const size_t VRAM_LIMIT = 12UL * 1024 * 1024 * 1024; // 12GB safe limit
    size_t estimated_memory = input_size * 4;
    if (estimated_memory > VRAM_LIMIT) return false;
    ...
    for (int run = 0; run < NUM_RUNS; run++) {
        ...
        std::cout << "      Run " << (run+1) << ": " << std::fixed << std::setprecision(2) << ms << " ms\n";
    }
    
    double total_time = 0.0;
    for (double t : run_times) total_time += t;
...
int main() {
    ...
    csv_file << "Formula,InputSize,BlockSize,Parallel,TimeMS,ThroughputMBps,Success\n";

    std::ofstream error_log("benchmark_errors.log");

    int total_tests = (block_sizes.size() + formulas.size()) * input_sizes.size() * parallel_modes.size();
    std::cout << "Total tests to run: " << total_tests 
              << " (sizes up to " << format_size(input_sizes.back()) 
              << ", blocks up to " << (block_sizes.back() / (1024*1024)) << "MB)\n\n";
    ...
    for (size_t size : input_sizes) {
        ...
        int group_tests = (int)((block_sizes.size() + formulas.size()) * parallel_modes.size());
        if (cumulative_tests > 0) {
            double avg_time_so_far = cumulative_avg_time / cumulative_tests; // ms per test
            ...
            std::cout << "Estimated time for this group: ~" 
                      << std::fixed << std::setprecision(1) << eta_seconds << " seconds\n";
        } else {
        ...
        for (u32 block_size : block_sizes) {
            for (bool parallel : parallel_modes) {
                if (!parallel && size >= 256 * 1024 * 1024) continue;
                if (block_size == 8 * 1024 * 1024 && size >= 256 * 1024 * 1024) continue;

                std::ostringstream block_stream;
                if (block_size >= 1024 * 1024) block_stream << (block_size / (1024 * 1024)) << "MB";
                else block_stream << (block_size / 1024) << "KB";
                ...
                bool ok = run_benchmark(name, size, block_size, parallel, result, test_number);

                if (ok && result.success) {
                    std::cout << "   Result: OK | Avg: " << std::fixed << std::setprecision(2) 
                              << result.time_ms << " ms | Throughput: " 
        ...
        for (const auto& formula : formulas) {
            ...
            for (bool parallel : parallel_modes) {
                ...
                std::cout << "\n[Test #" << test_number << "] Benchmarking " << formula.first 
                          << " (Block " << block_label << ") on " << format_size(size) 
                          << " (" << (parallel ? "Parallel" : "Serial") << ")\n";

                BenchmarkResult result;
                bool ok = run_benchmark(formula.first, size, block_size, parallel, result, test_number);
                ...
                if (ok && result.success) {
                    ...
                    csv_file << formula.first << "," << format_size(size) << "," << block_label << "," 
                             << (parallel ? "1" : "0") << "," 
                             << result.time_ms << "," << result.throughput_mbps << ",1\n";
                    ...
                    csv_file << formula.first << "," << format_size(size) << "," << block_label << "," 
                             << (parallel ? "1" : "0") << ",0,0,0\n";
                    fail_count++;
                    error_log << "FAILED: " << formula.first << " on " << format_size(size) 
                              << " (" << (parallel ? "Parallel" : "Serial") << ")\n";
                }
                csv_file.flush();
                error_log.flush();
    ...
    auto total_duration = std::chrono::duration_cast<std::chrono::seconds>(benchmark_end - benchmark_start).count();

    std::time_t start_time = std::chrono::system_clock::to_time_t(benchmark_start);
    ...
    std::cout << "  Total Tests Run: " << (success_count + fail_count) << "/" << total_tests << "\n";
