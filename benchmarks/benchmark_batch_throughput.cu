...
BenchmarkResult run_batch_benchmark(ZstdBatchManager &manager, size_t size,
  ...
  CUDA_CHECK(cudaMalloc(&d_input_base, total_input_size));
  CUDA_CHECK(cudaMalloc(&d_output_base, total_output_size));
  CUDA_CHECK(cudaMalloc(&d_decompressed_base, total_input_size));

  // Fill device input
  // Copy the single item pattern 'batch_count' times
  for (int i = 0; i < batch_count; i++) {
    CUDA_CHECK(cudaMemcpyAsync((uint8_t *)d_input_base + i * size,
...
BenchmarkResult run_graph_benchmark(ZstdBatchManager &manager, size_t size,
  ...
  // 3. Workspace (MUST BE PER ITEM for Graph/Async safety if sharing one huge
  // buffer?) Actually, compress() uses temp_workspace. If we launch N nodes in
  // a graph, they execute sequentially? CUDA Graph nodes are dependencies. If
  // we link them sequentially, we can reuse workspace? But compress_batch
  // loops. To be safe, we allocate N workspaces.
  ...
  CUDA_CHECK(cudaMalloc(&d_temp_all, one_temp_size));
  ...
  for (int i = 0; i < batch_count; i++) {
    // Data dependency: output_size is read/written. Since we are capturing, the
    // kernel writes to the pointer. We must ensure the pointer is valid DEVCIE
    // pointer or PINNED host? compress() signature: size_t *dst_size. If
    // dst_size is host memory, invalid for graph capture if it's not
    // pinned/mapped? Actually, normally compress() writes to it at end of
    // operation. DefaultZstdManager uses it. To be safe for Graph, let's use a
    // device buffer for output_sizes if needed, or assume Managed/Pinned. But
    // for now, we just pass &items[i].output_size. items is vector on host.
    // This might be the issue?
    // NO, let's stick to the OOM fix first: Reuse d_temp_all.

    // Using &dummy_local is bad because it's a stack variable that goes out of
    // scope? But capture captures the VALUE of the pointer.
    // items[i].output_size is a value in a vector.
    // We should pass a pointer that remains valid during execution.
    // The vector 'items' remains valid during execution loop.
    // BUT the 'pointer' passed to kernel is &items[i].output_size.
    // Is that pointer accessible from device? dst_size is usually a host
    // pointer for status. If it's used inside a kernel, it must be device
    // pointer. DefaultZstdManager::compress updates *dst_size = cSize. If that
    // code runs on host (after sync), good. But if we capture, the host code
    // runs AT CAPTURE time? NO, DefaultZstdManager::compress performs the logic
    // NOW. It launches kernels. It does `*compressed_size = cSize` at the end
    // on HOST. But `cSize` is calculated on CPU? WAIT. If `DefaultZstdManager`
    // calculates `cSize` on host, it means it ran on CPU? NO. If it runs on
    // GPU, how does it know `cSize`? Looking at line 1251 (viewed earlier):
    // `size_t cSize = ZSTD_compress(...)` -> This IS the CPU fallback path!
    // The code I viewed was `ExecutionPath::CPU` block inside
    // `SmartRouter::compress`? Just checked `cuda_zstd_manager.cu`: "Status
    // DefaultZstdManager::compress(...)". I need to see the GPU path.

    // For now, applying the OOM Fix (Reuse Workspace).
...
int main() {
  ...
  // Debug with small batch first
  /*
  std::vector<std::pair<size_t, int>> batch_cases = {
      {4096, 1},    // Debug
      {4096, 100},  // Small batch
      {4096, 2000}, // Target batch
  };

  for (auto &c : batch_cases) {
    std::cout << "Running size=" << c.first << " count=" << c.second << "..."
              << std::endl;
    auto result = run_batch_benchmark(*manager, c.first, c.second, stream);
    std::cout << std::setw(10) << c.first << " | " << std::setw(8) << c.second
              << " | " << std::setw(15) << std::fixed << std::setprecision(2)
              << result.compress_mbps << " | " << std::setw(15)
              << result.decompress_mbps << std::endl;
  }
  */
  ...
  for (auto &c : batch_cases) {
    ...
    for (int i = 0; i < batch_cnt; ++i) {
      CUDA_CHECK(cudaMalloc(&d_inputs[i], chunk_size));
      CUDA_CHECK(cudaMalloc(&d_outputs[i], chunk_size * 2));
      CUDA_CHECK(cudaMemcpy(d_inputs[i], h_pattern.data(), chunk_size,
