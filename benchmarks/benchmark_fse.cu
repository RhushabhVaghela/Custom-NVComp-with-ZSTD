...
void run_benchmark(size_t size, int iterations) {
    ...
    // We use the manager to compress, which uses FSE internally for literals/sequences
    // But wait, the manager does full Zstd compression. 
    // To benchmark *just* FSE decode, we should ideally call decode_fse directly.
    ...
    cuda_zstd::Status status = manager->compress(d_input, size, d_compressed, &compressed_size, d_temp, temp_size, nullptr, 0, 0);
    ...
    manager->decompress(d_compressed, compressed_size, d_output, &decompressed_size, d_temp, temp_size);
    ...
    std::cout << "  CPU Path: " << cpu_ms << " ms (" << cpu_gbps << " GB/s)" << std::endl;
    std::cout << "  GPU Path: " << gpu_ms << " ms (" << gpu_gbps << " GB/s)" << std::endl;
