...
__global__ void k_fse_decode(const u8 *__restrict__ bitstream,
  ...
  for (int byteIdx = (int)bitstreamSize - 1; byteIdx >= 0 && !found;
       byteIdx--) {
...
__host__ Status FSE_allocTableRFC(FSETableRFC &table, u32 tableLog,
  ...
  err = cudaMallocAsync(&table.d_entries,
                        table.tableSize * sizeof(FSEDistributionEntry), stream);
  if (err != cudaSuccess)
    return Status::ERROR_OUT_OF_MEMORY;

  err = cudaMallocAsync(&table.d_normFreqs, (maxSymbol + 1) * sizeof(u16),
                        stream);
  if (err != cudaSuccess) {
...
__host__ Status FSE_buildTableRFC(const u16 *normFreqs, u32 maxSymbol,
  ...
  k_fse_build_table<<<1, 256, 0, stream>>>(table.d_normFreqs, maxSymbol,
                                           tableSize, table.d_entries,
...
__host__ Status FSE_encodeRFC(const u8 *d_symbols, u32 numSymbols,
                              ...
                              cudaStream_t stream) {
  k_fse_encode<<<1, 1, 0, stream>>>(d_symbols, numSymbols, table.d_entries,
                                    table.d_symbolFirstState, table.tableLog,
                                    table.tableSize, d_bitstream,
                                    bitstreamCapacity, d_outputSize);
...
__host__ Status FSE_decodeRFC(const u8 *d_bitstream, size_t bitstreamSize,
  ...
  k_fse_decode<<<1, 1, 0, stream>>>(d_bitstream, bitstreamSize, table.d_entries,
                                    table.tableLog, numSymbols, d_symbols);
...
__global__ void k_fse_encode_rfc_from_old_tables(
  ...
  auto write_bits = [&](u64 val, u32 nbBits) {
    ...
    bitCount += nbBits;
    while (bitCount >= 8) {
      if (ptr < end_limit) *ptr++ = (unsigned char)(bitContainer & 0xFF);
      bitContainer >>= 8;
      bitCount -= 8;
    }
  };
  ...
  // RFC 8878: Start from the FIRST symbol and encode forwards
  // (Since we encode in forward order, the LAST symbol processed will be at the END of the bitstream)
  // (The decoder reads from the END, so it will see the LAST encoded symbol FIRST)
  u32 first_idx = 0;
  u32 stateLL = tables[0].d_symbol_first_state[d_ll_codes[first_idx]];
  u32 stateOF = tables[1].d_symbol_first_state[d_of_codes[first_idx]];
  u32 stateML = tables[2].d_symbol_first_state[d_ml_codes[first_idx]];

  // Write extra bits for the first sequence (read last by decoder)
  // Order: ML, OF, LL (Reverse of Decoder Read Order: LL, OF, ML)
  write_bits(d_ml_extras[first_idx], d_ml_bits[first_idx]);
  write_bits(d_of_extras[first_idx], d_of_bits[first_idx]);
  write_bits(d_ll_extras[first_idx], d_ll_bits[first_idx]);

  // Encode remaining symbols in forward order
  for (u32 i = 1; i < num_symbols; i++) {
    // RFC 8878 Interleaving: Decoder reads LL bits, then OF bits, then ML bits.
    // Thus, Encoder must write ML bits, then OF bits, then LL bits to the stack.

    // 1. Match Length FSE Bits
    u8 ml_code = d_ml_codes[i];
    u32 nbBitsML = (stateML + tables[2].d_symbol_table[ml_code].deltaNbBits) >> 16;
    write_bits(stateML & ((1u << nbBitsML) - 1), nbBitsML);
    stateML = (stateML >> nbBitsML) + tables[2].d_symbol_table[ml_code].deltaFindState;

    // 2. Offset FSE Bits
    u8 of_code = d_of_codes[i];
    u32 nbBitsOF = (stateOF + tables[1].d_symbol_table[of_code].deltaNbBits) >> 16;
    write_bits(stateOF & ((1u << nbBitsOF) - 1), nbBitsOF);
    stateOF = (stateOF >> nbBitsOF) + tables[1].d_symbol_table[of_code].deltaFindState;

    // 3. Literal Length FSE Bits
    u8 ll_code = d_ll_codes[i];
    u32 nbBitsLL = (stateLL + tables[0].d_symbol_table[ll_code].deltaNbBits) >> 16;
    write_bits(stateLL & ((1u << nbBitsLL) - 1), nbBitsLL);
    stateLL = (stateLL >> nbBitsLL) + tables[0].d_symbol_table[ll_code].deltaFindState;

    // 4. Extra Bits (Decoder reads LL, OF, ML - thus write ML, OF, LL)
...
__host__ Status launch_fse_encoding_kernel_rfc(
    ...
    cudaStream_t stream) {
  // Validate inputs
  if (!d_ll_codes || !d_of_codes || !d_ml_codes || !d_bitstream ||
      !d_output_pos || !d_tables) {
  ...
  // Zero the output buffer to ensure sentinel bit search works correctly
...
__host__ Status FSE_normalizeFreqs(const u32 *rawFreqs, u32 maxSymbol,
  ...
  for (u32 s = 0; s <= maxSymbol; s++) {
    if (rawFreqs[s] > 0) {
      // Scale with rounding
      scaled[s] =
          ((u64)rawFreqs[s] * tableSize + (numSamples / 2)) / numSamples;
      if (scaled[s] == 0)
        scaled[s] = 1; // Minimum 1
