 ...
 * 
 * WARNING: This implementation of Predefined Mode Encoder is flawed for numSymbols > 1.
 * 
 * Issues:
 * 1. It ignores input `symbols[i]` for i > 0. It assumes the state transitions (nextState) 
 *    in the table perfectly predict the next symbol in the sequence. This is only true 
 *    if the input sequence is a specific chain generated by the table structure, which 
 *    is not true for general compression.
 * 2. It uses `nextState` from the table as the transition target. In standard FSE, 
 *    `nextState` in a Decoder table (which this uses) is the Base Value for decoding, 
 *    NOT the Table Index for encoding the next symbol.
 * 3. Standard FSE encoding requires `deltaNbBits` and `deltaFindState` tables to 
 *    lookup the correct state range for an arbitrary next symbol. This kernel lacks that logic.
 * 
 * Current Status: 
 * - Works for numSymbols = 1 (single state transition).
 * - Produces garbage for numSymbols > 1.
 * 
 * Fix Required:
 * - Rewrite this kernel to use full FSE Encode Table logic (deltaNbBits/deltaFindState).
 * - Or ensure the input `table` provides the necessary lookup maps.
 */
