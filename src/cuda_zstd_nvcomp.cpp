...
int status_to_nvcomp_error(Status status) {
    switch (status) {
        ...
        case Status::ERROR_INVALID_PARAMETER: return 2;
        case Status::ERROR_OUT_OF_MEMORY: return 3;
        case Status::ERROR_CUDA_ERROR: return 4;
        case Status::ERROR_CORRUPT_DATA: return 6;
        case Status::ERROR_BUFFER_TOO_SMALL: return 7;
        case Status::ERROR_CHECKSUM_FAILED: return 10;
        case Status::ERROR_COMPRESSION: return 12;
        default: return 1; // ERROR_GENERIC
    }
}
...
Status nvcomp_error_to_status(int nvcomp_error) {
    switch (nvcomp_error) {
        ...
        case 2: return Status::ERROR_INVALID_PARAMETER;
        case 3: return Status::ERROR_OUT_OF_MEMORY;
        case 4: return Status::ERROR_CUDA_ERROR;
        case 6: return Status::ERROR_CORRUPT_DATA;
        case 7: return Status::ERROR_BUFFER_TOO_SMALL;
        case 10: return Status::ERROR_CHECKSUM_FAILED;
        case 12: return Status::ERROR_COMPRESSION;
        default: return Status::ERROR_GENERIC;
    }
}
...
bool is_nvcomp_v5_zstd_format(const void* compressed_data, size_t compressed_size) {
    if (compressed_size < 4) return false;
    
    u32 magic = 0;
    // We assume this might be called with a host pointer for metadata checks
    // If it's device, this memcpy is invalid, but is_nvcomp_* usually implies host access or specific API
    // For safety in this glue layer, we'll assume standard ZSTD magic behavior.
    memcpy(&magic, compressed_data, sizeof(u32));
    
    if (magic == ZSTD_MAGIC) return true;
    if ((magic & 0xFFFFFFF0) == ZSTD_MAGIC_SKIPPABLE_START) return true;
...
NvcompV5Options to_nvcomp_v5_opts(const CompressionConfig& config) {
    ...
    opts.enable_checksum = (config.checksum != ChecksumPolicy::NO_COMPUTE_NO_VERIFY);
    return opts;
}
...
CompressionConfig from_nvcomp_v5_opts(const NvcompV5Options& opts) {
    ...
    config.checksum = opts.enable_checksum ? 
...
std::unique_ptr<ZstdManager> create_nvcomp_v5_manager(const NvcompV5Options& opts) {
...
size_t NvcompV5BatchManager::get_compress_temp_size(
    const size_t* d_chunk_sizes,
    size_t num_chunks,
    cudaStream_t stream
) const {
...
size_t NvcompV5BatchManager::get_decompress_temp_size(
...
size_t NvcompV5BatchManager::get_max_compressed_chunk_size(size_t uncompressed_chunk_size) const {
...
Status NvcompV5BatchManager::decompress_async(
    ...
    Status status = pimpl_->manager->decompress_batch(
        items,
        d_temp_storage,
        temp_storage_bytes,
        stream
...
Status get_metadata_async(
    ...
    if (!h_metadata) return Status::ERROR_INVALID_PARAMETER;
    ...
    if (status != Status::SUCCESS && status != Status::ERROR_BUFFER_TOO_SMALL) return status;
...
Status get_decompressed_size_async(
    ...
    if (!h_decompressed_size) return Status::ERROR_INVALID_PARAMETER;
    
    NvcompV5Metadata metadata;
    Status status = get_metadata_async(d_compressed_data, compressed_size, &metadata, stream);
...
Status get_chunk_sizes(
    ...
    size_t last_chunk_size = metadata.uncompressed_size - (metadata.chunk_size * (metadata.num_chunks - 1));
...
int nvcomp_zstd_compress_async_v5(
    ...
    if (!manager) return status_to_nvcomp_error(Status::ERROR_INVALID_PARAMETER);
    
    // Compress without dictionary
    Status status = manager->compress(
        d_uncompressed, uncompressed_size,
...
int nvcomp_zstd_get_metadata_v5(
    ...
    Status status = get_metadata_async(d_compressed_data, compressed_size, h_metadata, stream);
    return status_to_nvcomp_error(status);
}
...
NvcompV5BenchmarkResult benchmark_level(
    ...
    result.compress_throughput_mbps = (input_size / (1024.0 * 1024.0)) / (result.compress_time_ms / 1000.0);
    
    // --- Benchmark Decompression ---
    size_t decomp_temp_size = manager->get_decompress_temp_size(result.compressed_size);
    ...
    for (int i = 0; i < iterations; ++i) {
        manager->decompress(
            ...
            d_temp, decomp_temp_size, stream
        );
    }
    cudaEventRecord(stop, stream);
    cudaEventSynchronize(stop);
    ...
    result.decompress_throughput_mbps = (input_size / (1024.0 * 1024.0)) / (result.decompress_time_ms / 1000.0);
...
std::vector<NvcompV5BenchmarkResult> benchmark_all_levels(
    ...
    for (int level = 1; level <= 22; ++level) {
        results.push_back(benchmark_level(d_input, input_size, level, iterations, stream));
    }
    return results;
}
