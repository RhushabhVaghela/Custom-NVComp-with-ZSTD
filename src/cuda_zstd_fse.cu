...
__host__ u32 select_optimal_table_log(const u32 *frequencies, u32 total_count,
  ...
  while ((1u << recommended) < unique_symbols * 2 &&
         recommended < FSE_MAX_TABLELOG)
    recommended++;
...
__host__ Status FSE_buildCTable_Host(
  ...
  // DEBUG: Dump spread table
  printf("[CTable SPREAD] First 20 state_to_symbol mappings:\n");
  for (u32 i = 0; i < min(20u, table_size); i++) {
    printf("[CTable SPREAD] state=%u symbol=%u\n", i, state_to_symbol[i]);
  }
  fflush(stdout);

  // === Step 3: Build tableU16 (d_next_state) ===
  // This is the cumulative sorted table: tableU16[cumul[s]++] = tableSize +
  // position We iterate through the SPREAD positions to fill the SORTED table
  ...
  for (u32 u = 0; u < table_size; u++) {
    ...
    printf("[CTable] Sym=%u deltaNbBits=%d deltaFindState=%d\n", s,
           h_table->d_symbol_table[s].deltaNbBits,
           h_table->d_symbol_table[s].deltaFindState);
  }
  ...
  for (u32 i = 0; i < input_size; ++i) {
    ...
    if (i < 12) {
      printf(
          ...
          "deltaFindState=%u, nbBitsOut=%u, nextStateIndex=%u, state_out=%u\n",
...
__global__ void fse_parallel_encode_kernel(
    const byte_t *d_input, u32 input_size,
    const FSEEncodeTable::FSEEncodeSymbol *d_symbol_table,
    const u16 *d_next_state, u32 table_log,
  ...
  for (u32 i = in_idx_start; i < in_idx_end; i++) {
    ...
    if (in_idx_end <= 20 && threadIdx.x == 0 && blockIdx.x == 0) {
      printf("[ENC] i=%u sym=%u st_in=%u bits=%u val=%u st_out=%u\n", i, symbol,
             old_state, nbBitsOut, val, state);
    }
  }
...
__global__ void fse_parallel_bitstream_copy_kernel(
  ...
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    printf("[BS_DUMP] Chunk 0 Input: %02X %02X %02X %02X\n", d_chunk_input[0],
           d_chunk_input[1], d_chunk_input[2], d_chunk_input[3]);
    // Also dump potential extra bytes just in case
    printf("[BS_DUMP] Chunk 0 Input (4-7): %02X %02X %02X %02X\n",
           d_chunk_input[4], d_chunk_input[5], d_chunk_input[6],
           d_chunk_input[7]);
  }
...
__host__ Status FSE_buildDTable_Host(const u16 *h_normalized, u32 max_symbol,
  ...
  // === Step 3: SYMBOL SPREADING - Custom Algorithm (matches encoder) ===
  for (u32 s = 0; s <= max_symbol; s++) {
  ...
  // === Step 4: Build DTable - Custom Formula (73% Success) ===
  for (u32 state = 0; state < table_size; state++) {
    ...
    // Calculate nbBits using CLZ (matches encoder)
    u32 clz_result;
#if defined(__GNUC__) || defined(__clang__)
    clz_result = __builtin_clz(freq);
#elif defined(_MSC_VER)
    ...
    h_table.nbBits[state] = (u8)maxBitsOut;

    // 73% Solution: Baseline with per-state offset and masking
    // This prevents overflow but causes 3/11 bytes to fail (documented)
    u32 baseline_raw = minStatePlus - table_size + state;
    h_table.newState[state] = (u16)(baseline_raw & (table_size - 1));
  }
  fflush(stdout);
...
__host__ Status encode_fse_advanced_debug(const byte_t *d_input, u32 input_size,
  ...
  printf("[DEBUG] Point D: Table Copy\n");
  fflush(stdout);
  CUDA_CHECK(cudaMemcpyAsync(d_dev_symbol_table, h_ctable.d_symbol_table,
                             sym_size_bytes, cudaMemcpyHostToDevice, stream));
  ...
  fse_parallel_encode_kernel<<<num_chunks, encode_threads, 0, stream>>>(
      d_input, input_size, d_dev_symbol_table, d_dev_next_state, table_log,
      num_chunks, d_chunk_start_states, d_bitstreams, d_chunk_bit_counts,
  ...
  // 7c: Scan (prefix sum of bit counts)
  if (num_chunks == 1) {
    // Optimization/Fix for single chunk: Scan is trivial (offset 0)
    // Also avoids parallel_scan implementation issues for n=1
    u32 zero = 0;
  ...
  printf("[DEBUG] Point I: Launch Copy\n");
  fflush(stdout);
...
__host__ Status encode_fse_batch(const byte_t **d_inputs,
  ...
  // Since we can't easily allocate variable size arrays in a loop without
  // fragmentation, we'll allocate one large chunk for all blocks if possible,
  // or per-block. Given we are inside a function, we should try to use the
  // provided output buffer if possible? No, FSE parallel encode needs
  // intermediate buffers (start states, bitstreams).
  ...
  for (u32 i = 0; i < num_blocks; i++) {
    ...
    if (h_block_types[i] == 1) {
      ...
      u32 max_symbol =
          0; // Marker for RLE (vs Raw which would be 255 or similar)
      u8 rle_symbol = h_rle_symbols[i];

      // Header: TableLog(4) + InputSize(4) + MaxSymbol(4) + Symbol(2 bytes
      // padding/value)
    ...
    // We don't know the exact size yet, but we can clear a safe amount or the
    // whole buffer if we knew the capacity. d_outputs[i] points to a buffer of
    // size d_output_sizes[i]? No, d_output_sizes is output. The user provides
    // d_outputs[i] with sufficient capacity. We'll clear the max possible size
    // (input_size * 2 is safe/conservative). Or just clear the part we will
    // write to? We don't know the size yet. But we know max_chunk_size *
    // num_chunks. Clear output buffer. We assume the user allocated enough
    // space. We clear input_size * 1.2 + margin to be safe and efficient.
    ...
    fse_parallel_encode_setup_kernel<<<1, 1, 0, stream>>>(
        d_inputs[i], input_size, d_table, d_next_state, table_log, num_chunks,
        d_block_states);

    fse_parallel_encode_kernel<<<num_chunks, 256, 0, stream>>>(
        d_inputs[i], input_size, d_table, d_next_state, table_log, num_chunks,
        d_block_states, d_block_bitstreams, d_block_counts, max_chunk_size);
...
find_chunk_boundaries_cpu(const byte_t *d_bitstream, u32 bitstream_size,
  ...
  // Decode chunks in reverse order (63 -> 0) because bitstream is concatenated
  // 0..63 and we read from the end. Within each chunk, we decode forward (0 ->
  // 4095).
...
__global__ void fse_parallel_decode_kernel_v2(
  ...
  if (chunk_id == 0) {
    printf(
        "GPU Decoder Chunk 0: start_seq=%u, count=%u, state=%u, bit_pos=%u\n",
