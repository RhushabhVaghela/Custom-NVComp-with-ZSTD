...
Status compress_with_workspace(const u8 *d_input, size_t input_size,
                               u8 *d_output, size_t *output_size,
  ...
  CUDA_CHECK_RETURN(
      cudaMemset(hash_table, 0xFF, workspace.hash_table_size * sizeof(u32)));
  ...
  CUDA_CHECK_RETURN(
      cudaMemcpy(h_input, d_input, input_size, cudaMemcpyDeviceToHost));
  ...
  while (in_pos < input_size) {
    ...
    while (in_pos + run_length < input_size &&
           h_input[in_pos + run_length] == current_byte && run_length < 255) {
      run_length++;
    }
  ...
  CUDA_CHECK_RETURN(cudaMemcpy(d_output, h_output,
                               final_compressed_size - sizeof(header),
                               cudaMemcpyHostToDevice));

  // Update header with actual compressed size
  header.compressed_size = final_compressed_size;
  CUDA_CHECK_RETURN(
      cudaMemcpy(d_output, &header, sizeof(header), cudaMemcpyHostToDevice));
...
int main() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    status = compress_with_workspace(d_input, input_size, d_output,
